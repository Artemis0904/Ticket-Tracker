...
export default function MRFormDialog({ trigger }: MRFormDialogProps) {
  ...
  const [zone, setZone] = useState<ZoneOption>('Zone A');
  ...
  const onSubmit = async () => {
    ...
    await notify({
      eventType: 'MR_CREATED_BY_ENGINEER',
      zone,
      request: { title: reqTitle, ticketNumber: ticketNumber.trim(), zone, description: description.trim(), requestedBy: state.currentUser || 'Engineer', requesterEmail: user?.email ?? undefined },
      targetDepartments: ['regional_manager'],
    });
